...
import CesiumDocumentContentProvider from './cs-document-content-provider';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const csProvider = new CesiumDocumentContentProvider();
    const olRegistration = vscode.workspace.registerTextDocumentContentProvider(extension.MAP_PREVIEW_URI_SCHEME, olProvider);
    const csRegistration = vscode.workspace.registerTextDocumentContentProvider(extension.CESIUM_PREVIEW_URI_SCHEME, csProvider);
    ...
    const csPreviewCommand = vscode.commands.registerCommand(extension.CESIUM_PREVIEW_COMMAND_ID, () => {
        const doc = vscode.window.activeTextEditor.document;
        const previewUri = extension.makePreviewUri("cesium", doc);
        csProvider.clearPreviewProjection(previewUri);
        csProvider.triggerVirtualDocumentChange(previewUri);
        vscode.commands.executeCommand('vscode.previewHtml', previewUri, vscode.ViewColumn.Two).then((success) => {
    ...
    context.subscriptions.push(mapPreviewCommand, mapPreviewWithProjCommand, csPreviewCommand, olRegistration);
}
